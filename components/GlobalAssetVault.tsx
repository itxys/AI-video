...
export const GlobalAssetVault: React.FC<GlobalAssetVaultProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-950/90 backdrop-blur-xl flex items-center justify-center p-4 md:p-10 animate-in fade-in duration-300">
      <div className="bg-slate-900 border border-slate-800 w-full max-w-[1440px] h-full max-h-[95vh] rounded-3xl flex flex-col overflow-hidden shadow-2xl relative">
        {/* Header */}
        <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-slate-900/50 backdrop-blur-md sticky top-0 z-40">
          <div>
        ...
        {/* Toolbar */}
        <div className="p-4 bg-slate-900 border-b border-slate-800 flex flex-wrap items-center gap-4 sticky top-[88px] z-40">
          <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-800">
            <button 
              onClick={() => { setActiveTab('chars'); setSelectedIds(new Set()); }}
              className={`px-8 py-2 rounded-lg text-xs font-black transition-all ${activeTab === 'chars' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}
            >
              {t.tabCharacters}
            </button>
            <button 
              onClick={() => { setActiveTab('items'); setSelectedIds(new Set()); }}
              className={`px-8 py-2 rounded-lg text-xs font-black transition-all ${activeTab === 'items' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}
            >
              {t.tabItems}
            </button>
          </div>
          ...
          <div className="flex-1 relative min-w-[300px]">
            <input 
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-10 pr-4 py-3 text-xs text-white outline-none focus:border-indigo-500 transition-all shadow-inner"
            />
          ...
          {selectedIds.size > 0 && (
            <button 
              ...
              className="px-6 py-2.5 bg-red-600/10 border border-red-500/20 text-red-500 rounded-xl text-xs font-black hover:bg-red-600 hover:text-white transition-all flex items-center gap-2"
            >
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-950/20">
          {activeTab === 'chars' ? (
            <div className="grid grid-cols-1 gap-12 max-w-5xl mx-auto pb-10">
                ...
                <div key={char.id} className="relative group">
                  <div className={`absolute top-6 left-6 z-20 transition-opacity ${selectedIds.has(char.id) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                    <input 
                      ...
                      className="w-6 h-6 rounded border-slate-700 bg-slate-900 text-indigo-600 focus:ring-indigo-500 cursor-pointer shadow-lg"
                    />
                  </div>
                  <CharacterBible 
                    profiles={[char]}
              ...
              {filteredCharacters.length === 0 && <p className="text-center text-slate-600 py-20 italic">{t.libEmpty}</p>}
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto pb-10">
                ...
                <div key={item.id} className="relative group">
                  <div className={`absolute top-4 left-4 z-20 transition-opacity ${selectedIds.has(item.id) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                    <input 
                      ...
                      className="w-5 h-5 rounded border-slate-700 bg-slate-900 text-indigo-600 focus:ring-indigo-500 cursor-pointer shadow-lg"
                    />
                  </div>
                  <ItemLibrary 
                    items={[item]}
               ...
               {filteredItems.length === 0 && <p className="col-span-full text-center text-slate-600 py-20 italic">{t.libEmpty}</p>}
            </div>
          )}
        </div>

        {/* Footer */}
        <div className="p-6 border-t border-slate-800 bg-slate-900 flex justify-end sticky bottom-0 z-40">
          <button 
            onClick={() => {
            ...
            className="px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-sm font-black shadow-xl shadow-indigo-600/20 active:scale-95 transition-all"
          >
